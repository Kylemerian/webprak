...
public class OrdersServiceTest {
    ...
    @Test
    public void testReadOrdersByUniqueBookId(){
        OrdersService ordersService = new OrdersService();
        Set<Orders> expected_list = Set.of(
                new Orders(1, java.sql.Date.valueOf("2020-02-17"), java.sql.Date.valueOf("2020-02-16"), 1),
                new Orders(1, java.sql.Date.valueOf("2019-02-17"), java.sql.Date.valueOf("2020-02-16"), 2)
        );
        for (Orders x:expected_list)
            ordersService.createOrder(x);
        List<Orders> list_of_orders = ordersService.readOrdersByUniqueBookId(2);
        Assert.assertEquals(list_of_orders.size(), 1);
        Assert.assertTrue(expected_list.contains(list_of_orders.get(0)));
        for (Orders x:expected_list)
            ordersService.deleteOrder(x);
    }
